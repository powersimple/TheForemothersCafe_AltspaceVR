...
public class ftBuildGraphics : ScriptableWizard
{
    ...
    static int voffset, soffset, ioffset, ioffset32o, ioffset32t, voffsetExport;
    ...
    struct TransparentSurf
    {
        public int id, offset32;
    };

    static VBFull fvbfull;
    static BufferedBinaryWriterFloat fvbtrace, fvbtraceTex, fvbtraceUV0;
    static BufferedBinaryWriterInt fib;
    static BinaryWriter fscene, fmesh, flmid, fseamfix, fsurf, fmatid, fmatide, fmatidh, fmatideb, fmeshlod, ftrilodmask, falphaid, fib32, fhmaps, finstances;
    static MemoryStream fsurfBuff;
    static List<TransparentSurf> fsurfTransparentIDs;
    ...
    {
        public uint version;
        public uint numUniqueMeshes;
        public uint numInstances;
    };

    struct InstanceDataUniqueMesh
    {
        //public uint voffset, vcount;
        public uint triOffset, triCount;
    };

    struct InstanceDataInstance
    {
        public float uvScaleX, uvScaleY, uvOffsetX, uvOffsetY;
        public float m00, m01, m02, m03, m04, m05, m06, m07, m08, m09, m10, m11;
        public uint meshID, objID;
    };

    class InstanceData
    {
        public InstanceDataHeader header;
        public List<InstanceDataUniqueMesh> uniqueMeshes;
        public List<bool> uniqueMeshIsTransparent;
        public List<InstanceDataInstance> instances;

        public InstanceData()
        {
            header = new InstanceDataHeader();
            header.version = 1;
            uniqueMeshes = new List<InstanceDataUniqueMesh>();
            uniqueMeshIsTransparent = new List<bool>();
            instances = new List<InstanceDataInstance>();
        }
    };

    struct InstanceHashData
    {
        public Mesh mesh;
        public int lmid;
        public ushort translucentAlphaID;
        public bool isFlipped;
        public bool castShadows;
        public byte lod;
        public int submesh;
        public bool submeshCastsShadows;
        //matid/alphaid/matide/matideb/matidh/mats/castshadows/ 
    };

    struct HashedUniqueMesh
    {
        public uint id;
    ...
    static Dictionary<InstanceHashData, HashedUniqueMesh> instanceToUnique;
    static uint instanceToUniqueMaxID, instanceDataTotalTriCount;//instanceDataTotalVCount
    static int[] instanceSkip;
    ...
    static Dictionary<int, BakeryNamedSection> idToSection;
    ...
    static List<GCHandle> tempPinnedHandles;
    ...
    public static HashSet<Renderer> forceScaleInLightmap0;

    public static bool traceInstances = false;

    static uint instanceUCounter;
    ...
    public static BakeryLightmapGroup GetLMGroupFromObject(GameObject obj)
    {
        ...
        if (lmgroup != null)
        {
            ...
            if (r)
            {
                ...
                if (forceScaleInLightmap0 != null && forceScaleInLightmap0.Contains(r)) scaleInLm = 0;
    ...
    static void exportSurfs(BinaryWriter f, int[][] indices, int subMeshCount, bool isTranslucent, ushort translucentAlphaID, List<ushort> alphaIDs)// Mesh m)
    {
        int offset = ioffset;

        int offset32o = ioffset32o;
        int offset32t = ioffset32t;

        for(int i=0;i<subMeshCount;i++)
        {
            var alphaID = isTranslucent ? translucentAlphaID : alphaIDs[(alphaIDs.Count - subMeshCount) + i];
            bool transparent = alphaID != 0xFFFF;
            if (transparent)
            {
                var t = new TransparentSurf();
                t.id = soffset + i;
                t.offset32 = offset32t;
                fsurfTransparentIDs.Add(t);
            }

            int size = indices[i].Length;//m.GetTriangles(i).Length;
            f.Write(offset);
            f.Write(size);
            f.Write(transparent ? offset32t : offset32o);
            offset += size;// * 2;
    ...
    {
        f.Write(voffsetExport);
        f.Write(soffset);
        f.Write((ushort)m.subMeshCount);
        f.Write((ushort)0);
    }

    static int getFinalLMID(GameObject obj, BakeryLightmapGroup lmgroup)//, out bool isTranslucent)
    {
        //isTranslucent = false;
    ...
    static void exportFinalLMID(BinaryWriter f, int lmid)
    {
        if (lmid == -int.MaxValue)
        {
            f.Write(0xFFFFFFFF);
        }
        else
    ...
    public static void ProgressBarEnd(bool removeTempObjects)// bool isError = true)
    {
        if (removeTempObjects)
        {
            ...
            if (tempPinnedHandles != null)
            {
                for(int i=0; i<tempPinnedHandles.Count; i++)
                {
                    //if (tempPinnedHandles[i] != null)
                    if (tempPinnedHandles[i].IsAllocated)
                    {
                        tempPinnedHandles[i].Free();
                    }
                }
                tempPinnedHandles = new List<GCHandle>();
            }
        }
    ...
    static void CloseAllFiles()
    {
        ...
        if (finstances != null) finstances.Close();
        ...
        fib = null;
        fscene = fmesh = flmid = fsurf = fmatid = fmatide = fmatideb = fmeshlod = ftrilodmask = falphaid  = fib32 = fseamfix = fmatidh = fhmaps = finstances = null;
    ...
    public static TexInput InputDataFromTex(Texture tex, TexInputType ttype = TexInputType.Color)
    {
        ...
        tempPinnedHandles.Add(handle);
        a.data = handle.AddrOfPinnedObject();
    ...
    static void ConvertTerrain(GameObject obj)
    {
        ...
        bool drawTreesAndFoliage = terr.drawTreesAndFoliage;
        ...
        terr.drawTreesAndFoliage = drawTreesAndFoliage;
        ...
        if (exportTerrainTrees && terr.drawTreesAndFoliage)
        {
            ...
            {
                Vector3 pos = Vector3.Scale(trees[t].position, tdata.size) + objPos;

                var treeProt = tdata.treePrototypes[trees[t].prototypeIndex];
                var prefab = treeProt.prefab;

                var newObj = GameObject.Instantiate(prefab, pos, Quaternion.AngleAxis(trees[t].rotation * Mathf.Rad2Deg, Vector3.up)) as GameObject;
                ...
                var ls = xform.localScale;
                xform.localScale = new Vector3(ls.x *  trees[t].widthScale, ls.y *  trees[t].heightScale, ls.z *  trees[t].widthScale);
            }
        }
    }
#endif
...
//#endif
    ...
    static int GetLightmapTag(GameObject obj, ExportSceneData data)
    {
        ...
        if (splitByTag && BakeryNamedSectionSelector.any)
        {
            var section = obj.GetComponentInParent<BakeryNamedSectionSelector>();
            if (section != null && section.sectionAsset != null)
            {
#if UNITY_6000_5_OR_NEWER
                tag = (int)(EntityId.ToULong(section.sectionAsset.GetEntityId())); // highly unlikely anything important will get cropped when converting...
#else
                tag = section.sectionAsset.GetInstanceID();
#endif
                idToSection[tag] = (BakeryNamedSection)section.sectionAsset;
            }
        }
...
/*#if UNITY_2019_1_OR_NEWER
        ...
        foreach(GameObject obj in objects)
        {
            ...
            var scaleInLm = so.FindProperty("m_ScaleInLightmap").floatValue;
            if (forceScaleInLightmap0 != null && forceScaleInLightmap0.Contains(mr)) scaleInLm = 0;
#if UNITY_2019_2_OR_NEWER
            var _r = mr as MeshRenderer;
            if (pstorage.takeReceiveGIIntoAccount && _r != null && _r.receiveGI == ReceiveGI.LightProbes) scaleInLm = 0;
#endif
            ...
            if (scaleInLm > 0)
            {
                ...
                else
                {
                    if (splitByTag && BakeryNamedSectionSelector.any)
                    {
                        ...
                        if (section != null && section.sectionAsset != null)
                        {
                            var s = (BakeryNamedSection)section.sectionAsset;
                            objToScaleInLm[obj] *= s.texelsPerUnitScale;
                        }
                    }
                }
            }
...
/*#if DAN
                ...
                if (uvPaddingPreserveIfExisted)
                {
                    ...
                    if (importer != null)
                    {
                        ...
                        {
                            if (ftLightmaps.PropIsBakery(props[p]))
                            {
                                return;
                            }
                        }
    ...
    static bool ValidateScaleOffsetImmutability(ExportSceneData data)
    {
        if (validateLightmapStorageImmutability)
        {
            ...
            for(int i=0; i<objsToWrite.Count; i++)
            {
                ...
                if (Mathf.Abs(scaleOffset.x - storedScaleOffset.x) > eps || 
                    Mathf.Abs(scaleOffset.y - storedScaleOffset.y) > eps ||
                    Mathf.Abs(scaleOffset.z - storedScaleOffset.z) > eps ||
                    Mathf.Abs(scaleOffset.w - storedScaleOffset.w) > eps)
                {
    ...
    static string GetObjectSceneIndexString(Transform obj)
    {
        string i = obj.GetSiblingIndex() + "";
        while(obj.parent != null)
        {
            obj = obj.parent;
            i = obj.GetSiblingIndex() + "_" + i;
        }
        return i;
    }
            ...
            if (comparisonResult == 0)
            {
                ...
                if (comparisonResult == 0)
                {
                    ...
                    if (comparisonResult == 0)
                    {
                        ...
                        if (comparisonResult == 0)
                        {
                            var aindexStr = GetObjectSceneIndexString(a.transform);
                            var bindexStr = GetObjectSceneIndexString(b.transform);
                            comparisonResult = bindexStr.CompareTo(aindexStr);
                        }
                    }
                }
            }
    ...
    static void MoveObjectToImplicitGroup(GameObject holder, BakeryLightmapGroup newGroup, ExportSceneData data)
    {
        ...
        var storageId = sceneToID[scn];
        var implicitGroupedObjects = storages[storageId].implicitGroupedObjects;
        var implicitGroups = storages[storageId].implicitGroups;
        
        for(int k=0; k<implicitGroupedObjects.Count; k++)
        {
            if (implicitGroupedObjects[k] == holder)
            {
                implicitGroups[k] = newGroup;
                //Debug.LogError("Implicit set: " + k+" "+newGroup.name+" "+holder.name);
            }
        }
    }
    ...
    static public IEnumerator ExportScene(EditorWindow window, bool renderTextures = true, bool atlasOnly = false, BakerySectorCapture sectorCaptureAsset = null)//, bool allowOptimizedLODs = true)
    {
        ...
        var instanceHashData = new InstanceHashData();
        var udesc = new InstanceDataUniqueMesh();
        var idesc = new InstanceDataInstance();
        if (traceInstances)
        {
            instanceData = new InstanceData();
            instanceToUnique = new Dictionary<InstanceHashData, HashedUniqueMesh>();
        }
        else
        {
            instanceToUnique = null;

        }
        instanceToUniqueMaxID = instanceDataTotalTriCount = 0;//instanceDataTotalVCount
        instanceUCounter = 0;
        var submeshCastsShadowsArray = new List<bool>();

        if (splitByTag && BakeryNamedSectionSelector.any) idToSection = new Dictionary<int, BakeryNamedSection>();
        ...
        try
        {
            ...
            //{
                ...
                if (splitByTag && BakeryNamedSectionSelector.any)
                {
                    // Apply Named Section settings
                    for(int i=0; i<groupList.Count; i++)
                    {
                        var grp = groupList[i];
                        if (grp.isImplicit && grp.tag != -1)
                        {
                            BakeryNamedSection section;
                            if (idToSection.TryGetValue(grp.tag, out section))
                            {
                                grp.name = section.name + "_" + grp.name;
                            }
                        }
                    }
                }
                ...
                for(int i=0; i<groupList.Count; i++)
                {
                    ...
                    if (uvgbflags != 0 && (uvgbGlobalFlags & UVGBFLAG_LOD) != 0) uvgbflags |= UVGBFLAG_LOD;
                ...
                voffset = ioffset = soffset = ioffset32o = ioffset32t = voffsetExport = 0; // vertex/index/surface write
                ...
                fsurfBuff = new MemoryStream();
                fsurfTransparentIDs = new List<TransparentSurf>();
                fsurf = new BinaryWriter(fsurfBuff);
                ...
                finstances = new BinaryWriter(File.Open(scenePath + "/instances.bin", FileMode.Create));
                ...
                if (ftRenderLightmap.clientMode)
                {
                    ...
                    ftClient.serverFileList.Add("instances.bin");
                ...
                if (traceInstances)
                {
                    instanceSkip = new int[objsToWrite.Count/32+1];
                }
                else
                {
                    instanceSkip = null;
                }
                uint instanceVOffset = 0;
                ...
                for(int i=0; i<objsToWrite.Count; i++)
                {
                    ...
                    int id = getFinalLMID(obj, lmgroup);
                    voffsetExport = voffset;
                    //exportMesh(fmesh, m);
                    ...
                    for(int k=0;k<m.subMeshCount;k++)
                    {
                        ...
                        if (submeshCastsShadowsArray.Count <= k)
                        {
                            submeshCastsShadowsArray.Add(submeshCastsShadows);
                        }
                        else
                        {
                            submeshCastsShadowsArray[k] = submeshCastsShadows;
                        }
                    }
                    ...
                    if (traceInstances)
                    {
                        ...
                        if (holderObj != null)
                        {
                            if (holderRect.TryGetValue(holderObj, out rc)) hasRC = true;
                        }
                        bool firstSubmesh = true;
                        bool skipThis = false;
                        uint soffsetO = 0;
                        uint soffsetT = 0;
                        HashedUniqueMesh uniqueMesh;

                        bool anyNonMatchingSubmesh = false;
                        for(int submeshID=0; submeshID<m.subMeshCount; submeshID++)
                        {
                            var alphaID = isTranslucent ? translucentAlphaID : alphaIDs[(alphaIDs.Count - m.subMeshCount) + submeshID];

                            instanceHashData.mesh = m;
                            instanceHashData.lmid = id;
                            instanceHashData.isFlipped = isFlipped;
                            instanceHashData.translucentAlphaID = alphaID;
                            instanceHashData.lod = wlod;
                            instanceHashData.submesh = submeshID;
                            instanceHashData.submeshCastsShadows = submeshCastsShadowsArray[submeshID];

                            if (!instanceToUnique.TryGetValue(instanceHashData, out uniqueMesh))
                            {
                                anyNonMatchingSubmesh = true;
                            }
                        }
                        ...
                        for(int submeshID=0; submeshID<m.subMeshCount; submeshID++)
                        {
                            ...
                            if (anyNonMatchingSubmesh)//!instanceToUnique.TryGetValue(instanceHashData, out uniqueMesh))
                            {
                                uniqueMesh.id = instanceUCounter;//(uint)instanceToUnique.Count;
                                instanceUCounter++;
                                uniqueMesh.objID = (uint)i;
                                uniqueMesh.matrix = tform.worldToLocalMatrix;

                                if (!hasRC)
                                {
                                    uniqueMesh.uvScaleOffset = new Vector4(1,1,0,0);
                                }
                                else
                                {
                                    uniqueMesh.uvScaleOffset = new Vector4(rc.width, rc.height, rc.x, rc.y);
                                }

                                instanceToUnique[instanceHashData] = uniqueMesh;
                                instanceToUniqueMaxID = uniqueMesh.id + 1;

                                if (!submeshCastsShadowsArray[submeshID])
                                {
                                    udesc.triOffset = udesc.triCount = 0;
                                }
                                else
                                {
                                    var indicesOut = alphaID == 0xFFFF ? indicesOpaque : indicesTransparent;

                                    udesc.triOffset = (uint)indicesOut.Count/3 + (alphaID != 0xFFFF ? soffsetT : soffsetO);
                                    udesc.triCount = (uint)inds[submeshID].Length/3;
                                }

                                instanceDataTotalTriCount += udesc.triCount;
                                if (alphaID != 0xFFFF)
                                {
                                    soffsetT += udesc.triCount;
                                }
                                else
                                {
                                    soffsetO += udesc.triCount;
                                }

                                instanceData.header.numUniqueMeshes++;
                                instanceData.uniqueMeshes.Add(udesc);
                                instanceData.uniqueMeshIsTransparent.Add(alphaID != 0xFFFF);

                                if (firstSubmesh)
                                {
                                    currentVoffset = voffsetExport = (int)instanceVOffset;
                                    instanceVOffset += (uint)objsToWriteVerticesPosW[i].Length;
                                    firstSubmesh = false;
                                }
                            }
                            else
                            {
                                instanceToUnique.TryGetValue(instanceHashData, out uniqueMesh);

                                idesc.meshID = uniqueMesh.id;
                                idesc.objID = uniqueMesh.objID;

                                if (!hasRC)
                                {
                                    idesc.uvScaleX = idesc.uvScaleY = 1;
                                    idesc.uvOffsetX = idesc.uvOffsetY = 0;
                                }
                                else
                                {
                                    //uv = (((uv*scale+offset)-offset)/scale)*scale2+offset2
                                    //uv = ((uv*scale+offset)-offset)*(scale2/scale)+offset2
                                    //uv = (uv*scale+offset)*(scale2/scale) - offset*(scale2/scale)+offset2

                                    float mulx = rc.width  / uniqueMesh.uvScaleOffset.x;
                                    float muly = rc.height / uniqueMesh.uvScaleOffset.y;

                                    float addx = -uniqueMesh.uvScaleOffset.z * mulx + rc.x;
                                    float addy = -uniqueMesh.uvScaleOffset.w * muly + rc.y;


                                    idesc.uvScaleX = mulx;
                                    idesc.uvScaleY = muly;
                                    idesc.uvOffsetX = addx;
                                    idesc.uvOffsetY = addy;
                                }
                                
                                var mtx = tform.localToWorldMatrix * uniqueMesh.matrix;
                                idesc.m00 = mtx[0, 0];
                                idesc.m01 = mtx[0, 1];
                                idesc.m02 = mtx[0, 2];
                                idesc.m03 = mtx[0, 3];
                                ...
                                instanceData.header.numInstances++;
                                instanceData.instances.Add(idesc);


                                instanceSkip[i>>5] |= 1 << (i & 31);
                                skipThis = true;
                            }
                        }
                    ...
                    exportFinalLMID(flmid, id);
                    exportMesh(fmesh, m);
                    exportSurfs(fsurf, inds, inds.Length, isTranslucent, translucentAlphaID, alphaIDs);// m);
                    ...
                    for(int k=0;k<m.subMeshCount;k++)
                    {
                        ...
                        if (submeshCastsShadowsArray[k])
                        {
                            ...
                            if (alphaID == 0xFFFF)
                            {
                                ioffset32o += indexCount;
                            }
                            else
                            {
                                ioffset32t += indexCount;
                            }
                        }
                        ioffset += indexCount;
                    }
        ...
        try
        {
                ...
                for(int i=0; i<objsToWrite.Count; i++)
                {
                    ...
                    if (traceInstances)
                    {
                        if ((instanceSkip[i>>5] & (1 << (i & 31))) != 0) doWrite = false;
                    }

                    if (doWrite)
                    {
                        objsToWriteFinalCount++;
                ...
                var _fsurf = File.Create(scenePath + "/surf.bin");
                if (_fsurf != null)
                {
                    int stride = 3 * sizeof(int);
                    int offset32structStart = 2 * sizeof(int);
                    for(int k=0; k<fsurfTransparentIDs.Count; k++)
                    {
                        int offs = fsurfTransparentIDs[k].id * stride + offset32structStart;
                        fsurf.BaseStream.Seek(offs, SeekOrigin.Begin);
                        fsurf.Write(fsurfTransparentIDs[k].offset32 + indicesOpaqueCount);
                    }

                    var _data = fsurfBuff.GetBuffer();
                    _fsurf.Write(_data, 0, _data.Length);
                    _fsurf.Close();
                }
                ...
                int meshID = 0;
                uint[] oldIDToShortOldID = null;
                fscene.Write(objsToWriteFinalCount);
                for(int i=0; i<objsToWrite.Count; i++)
                {
                    if (traceInstances)
                    {
                        if (oldIDToShortOldID == null) oldIDToShortOldID = new uint[objsToWrite.Count];
                        if ((instanceSkip[i>>5] & (1 << (i & 31))) != 0) continue;
                        oldIDToShortOldID[i] = (uint)meshID;
                    }
                    fscene.Write(meshID);
                    meshID++;
                }
                for(int i=0; i<objsToWrite.Count; i++)
                {
                    ...
                    fscene.Write(objsToWrite[i].name);
                }
                ...
                if (traceInstances)
                {
                    ...
                    // compact
                    var oldIDToNewID = new uint[instanceToUniqueMaxID];
                    var uniqueIDList = new uint[instanceToUniqueMaxID];
                    uint usedIDsCount = 0;
                    var instanceData2 = new InstanceData();
                    instanceData2.header.version = instanceData.header.version;
                    instanceData2.header.numInstances = instanceData.header.numInstances + 1; // first instance is non-instanced scene

                    // Add the main scene instance
                    idesc = new InstanceDataInstance();
                    idesc.m00 = 1; // identity
                    idesc.m05 = 1;
                    idesc.m10 = 1;
                    idesc.uvScaleX = 1;
                    idesc.uvScaleY = 1;
                    instanceData2.instances.Add(idesc);

                    for(int i=0; i<instanceData.header.numInstances; i++)
                    {
                        uint id1 = instanceData.instances[i].meshID;
                        idesc = instanceData.instances[i];
                        idesc.objID = oldIDToShortOldID[(int)instanceData.instances[i].objID];
                        if (oldIDToNewID[id1] == 0)
                        {
                            uniqueIDList[usedIDsCount] = id1;
                            usedIDsCount++;
                            oldIDToNewID[id1] = usedIDsCount; // start from 1 because 0 is the non-instanced scene
                            idesc.meshID = usedIDsCount;
                            instanceData2.instances.Add(idesc);
                        }
                        else
                        {
                            idesc.meshID = oldIDToNewID[id1];
                            instanceData2.instances.Add(idesc);
                        }
                    }
                    instanceData2.header.numUniqueMeshes = usedIDsCount + 1; // also add the non-instanced scene
                    
                    // The non-instanced scene
                    //udesc.voffset = 0;
                    //udesc.vcount = instanceDataTotalVCount;
                    udesc.triOffset = 0;
                    udesc.triCount = instanceDataTotalTriCount;
                    instanceData2.uniqueMeshes.Add(udesc);

                    // Add mesh ranges for each type of instance
                    for(int i=0; i<(int)usedIDsCount; i++)
                    {
                        int id1 = (int)uniqueIDList[i];
                        if (oldIDToNewID[id1] == 0) continue;

                        //udesc.vcount = instanceData.uniqueMeshes[id1].vcount;
                        //udesc.voffset = instanceData.uniqueMeshes[id1].voffset;
                        udesc.triOffset = instanceData.uniqueMeshes[id1].triOffset;
                        if (instanceData.uniqueMeshIsTransparent[id1])
                        {
                            udesc.triOffset += (uint)indicesOpaqueCount/3;
                        }
                        udesc.triCount = instanceData.uniqueMeshes[id1].triCount;
                        instanceData2.uniqueMeshes.Add(udesc);
                    }

                    // possibly old Mono version + possibly banned unsafe context leave us with shit methods
                    finstances.Write(instanceData2.header.version);
                    finstances.Write(instanceData2.header.numUniqueMeshes);
                    finstances.Write(instanceData2.header.numInstances);
                    for(int i=0; i<instanceData2.header.numUniqueMeshes; i++)
                    {
                        //finstances.Write(instanceData2.uniqueMeshes[i].voffset);
                        //finstances.Write(instanceData2.uniqueMeshes[i].vcount);
                        finstances.Write(instanceData2.uniqueMeshes[i].triOffset);
                        finstances.Write(instanceData2.uniqueMeshes[i].triCount);
                    }
                    for(int i=0; i<instanceData2.header.numInstances; i++)
                    {
                        finstances.Write(instanceData2.instances[i].uvScaleX);
                        finstances.Write(instanceData2.instances[i].uvScaleY);
                        finstances.Write(instanceData2.instances[i].uvOffsetX);
                        finstances.Write(instanceData2.instances[i].uvOffsetY);
                        finstances.Write(instanceData2.instances[i].m00);
                        finstances.Write(instanceData2.instances[i].m01);
                        finstances.Write(instanceData2.instances[i].m02);
                        finstances.Write(instanceData2.instances[i].m03);
                        finstances.Write(instanceData2.instances[i].m04);
                        finstances.Write(instanceData2.instances[i].m05);
                        finstances.Write(instanceData2.instances[i].m06);
                        finstances.Write(instanceData2.instances[i].m07);
                        finstances.Write(instanceData2.instances[i].m08);
                        finstances.Write(instanceData2.instances[i].m09);
                        finstances.Write(instanceData2.instances[i].m10);
                        finstances.Write(instanceData2.instances[i].m11);
                        finstances.Write(instanceData2.instances[i].meshID);
                        finstances.Write(instanceData2.instances[i].objID);
                    }
                    DebugLogInfo("Wrote instances in " + ((GetTime() - s)/1000.0) + "s");
                }
        ...
        if (exportShaderColors && renderTextures)
        {
            ...
            float maxEmission = 0;
            List<Texture2D> postSaveTex = null;
            List<string> postSaveName = null;
            List<bool> postSaveVBake = null;
            if (ftRenderLightmap.adaptiveLightScale)
            {
                postSaveTex = new List<Texture2D>();
                postSaveName = new List<string>();
                postSaveVBake = new List<bool>();
            }
            ...
            for(int g=0; g<groupList.Count; g++)
            {
                ...
                if (hasEmissive)
                {
                    ...
                    if (ftRenderLightmap.adaptiveLightScale)
                    {
                        emissive.name = groupList[g].name;
                        maxEmission = Mathf.Max(maxEmission, ftUVGBufferGen.FindMaxValue(emissive) * ftRenderLightmap.hackEmissiveBoost);
                    }
                    else
                    {
                        emissive = ProcessEmissive(emissive, vertexBake);
                    }
                }
                if (!vertexBake) ftUVGBufferGen.Dilate(albedo, 0);

                SaveGBufferTexture(albedo, "/uvalbedo_" + groupList[g].name, TexInputType.Color, isDX11);
                if (isDX11) yield return null;
                ...
                if (hasEmissive)
                {
                    if (ftRenderLightmap.adaptiveLightScale)
                    {
                        postSaveTex.Add(emissive);
                        postSaveName.Add("/uvemissive_" + groupList[g].name);
                        postSaveVBake.Add(vertexBake);
                    }
                    else
                    {
                        SaveGBufferTexture(emissive, "/uvemissive_" + groupList[g].name, TexInputType.HalfColor, isDX11);
                        if (isDX11) yield return null;
            ...
            if (ftRenderLightmap.adaptiveLightScale)
            {
                Debug.Log("Max emission: "+maxEmission);
                ftRenderLightmap.ComputeLightScale(maxEmission);
                for(int i=0; i<postSaveTex.Count; i++)
                {
                    postSaveTex[i] = ProcessEmissive(postSaveTex[i], postSaveVBake[i], true);
                    SaveGBufferTexture(postSaveTex[i], postSaveName[i], TexInputType.HalfColor, isDX11);
                    if (isDX11) yield return null;
                    DestroyImmediate(postSaveTex[i]);
                }
            }
        }
    ...
    static void SaveGBufferTexture(Texture2D tex, string name, TexInputType type, bool isDX11)
    {
        if (isDX11)
        {
            SaveGBufferMap(tex.GetNativeTexturePtr(),
                scenePath + name + (ftRenderLightmap.compressedGBuffer ? ".lz4" : ".dds"),
                ftRenderLightmap.compressedGBuffer);
            GL.IssuePluginEvent(5);
        }
        else
        {
            var bytes = InputBytesFromTex(tex, type);
            SaveGBufferMapFromRAM(bytes, bytes.Length,
                scenePath + name + (ftRenderLightmap.compressedGBuffer ? ".lz4" : ".dds"),
                ftRenderLightmap.compressedGBuffer);
        }
    ...
    static Texture2D ProcessEmissive(Texture2D emissive, bool vertexBake, bool force = false)
    {
        if (force || ftRenderLightmap.hackEmissiveBoost != 1.0f || ftRenderLightmap.lightScale != 1.0f) // lightScale forces float RT so must be checked separately
        {
            float totalMultiplier = ftRenderLightmap.hackEmissiveBoost * ftRenderLightmap.lightScale;
            emissive = ftUVGBufferGen.Multiply(emissive, totalMultiplier);
        }
