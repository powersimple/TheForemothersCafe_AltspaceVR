...
public class ftShaderTweaks : ScriptableWizard
{
    ...
    bool CopyInclude(string shadersDir)
    {
        ...
        {
            UnityEngine.Debug.LogError("Can't access Unity built-in shaders - please try running Unity as admin");
            return false;
        }
        return true;
    }
    ...
    void OnGUI()
    {
        if (!initialized)
        {
            try
            {
                ...
                var managedDir = System.IO.Path.GetDirectoryName(entryAssembly.GetLoadedAssemblyPath());
#else
                ...
                if (!Directory.Exists(shadersDir)) shadersDir = managedDir + "/../../Resources/CGIncludes/";
        ...
        if (initialized)
        {
            ...
            {
                if (!CopyInclude(shadersDir)) return;
                var reader = new StreamReader(includeGIPath);
                if (reader == null)
                {
                    UnityEngine.Debug.LogError("Can't open " + includeGIPath);
