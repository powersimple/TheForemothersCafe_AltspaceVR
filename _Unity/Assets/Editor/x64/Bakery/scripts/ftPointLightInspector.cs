...
public class ftPointLightInspector : UnityEditor.Editor
{
    ...
    ftInstanceID texCached = new ftInstanceID();
    ftInstanceID tex2DCached = new ftInstanceID();
    ftInstanceID iesCached = new ftInstanceID();
    ...
    void TestPreviewRefreshProperty(ref ftInstanceID cached, ftInstanceID newVal)
    {
        if (cached.NonEmpty())
        {
            if (cached != newVal)
    ...
    void TestPreviewRefreshProperty(ref ftInstanceID cached, UnityEngine.Object newVal)
    {
        if (newVal == null)
        {
            TestPreviewRefreshProperty(ref cached, ftInstanceID.empty);
            return;
        }
        TestPreviewRefreshProperty(ref cached, ftInstanceID.GetID(newVal));
    }
