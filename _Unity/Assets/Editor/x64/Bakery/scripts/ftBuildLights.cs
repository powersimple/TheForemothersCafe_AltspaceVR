...
public class ftBuildLights
{
    ...
    static public void BuildDirectLight(BakeryDirectLight obj, int SAMPLES, bool ignoreNormal = false, string outName = "direct.bin")
    {
        ...
        float intensity = obj.intensity * ftRenderLightmap.lightScale;

        #if SRGBCONVERT
            f.Write(obj.color.linear.r * intensity);
            f.Write(obj.color.linear.g * intensity);
            f.Write(obj.color.linear.b * intensity);
        #else
            f.Write(obj.color.r * intensity);
            f.Write(obj.color.g * intensity);
            f.Write(obj.color.b * intensity);
        #endif
        ...
        List<Vector3> vplData = null)
    {
        ...
        for(int sample=0; sample<SAMPLES; sample++)
        {
            ...
            if (corners == null)
            {
                ...
                trinormal = normalMatrix.MultiplyVector(trinormal).normalized;
            }
                                    ...
                                    int sampleDiv, bool ignoreNormal = false, string outName = "batchpointlight.bin")
    {
        ...
        {
            if (skipLight == null || !skipLight[L]) numLights++;
        }
        flights.Write(numLights);

        for(int i=start; i<=end; i++)
        {
            if (skipLight != null && skipLight[i]) continue;
            ...
            flights.Write(obj.color.linear.r * intensity);
            flights.Write(obj.color.linear.g * intensity);
            flights.Write(obj.color.linear.b * intensity);
        ...
        for(int L=start; L<=end; L++)
        {
            if (skipLight != null && skipLight[L]) continue;
