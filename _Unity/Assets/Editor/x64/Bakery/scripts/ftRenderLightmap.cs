...
public class ftRenderLightmap : EditorWindow//ScriptableWizard
{
    ...
    [DllImport ("halffloat2vb", CallingConvention=CallingConvention.Cdecl)]
    public static extern int halffloat2vbm([MarshalAs(UnmanagedType.LPWStr)]string inputFilename, System.IntPtr values, int dataType, float multiplier);
    ...
    public static bool hackAOProbes = false;
    ...
    public static bool adaptiveLightScale = false;
    public static float lightScale = 1.0f;

    static int serverBakeProgress;

    public const string ftraceExe6 = "ftraceRTX.exe";//"ftraceRTX.exe";
    public const string ftraceExe1 = "ftrace.exe";
    public const string ftraceExe9 = "ftraceRTX9.exe";
    ...
    static readonly string[] shEndingsNormal = {"L0", "L1x", "L1y", "L1z"};
    static readonly string[] shEndingsAOOnly = {"RNM0", "RNM1", "RNM2", "RNM3"};

    bool EndingAOOnly()
    {
        //return ftAdditionalConfig.bakeAOToProbes && userRenderMode == RenderMode.AmbientOcclusionOnly;
        return hackAOProbes && userRenderMode == RenderMode.AmbientOcclusionOnly;
    }

    static string EndingSH(bool ao, int coeff)
    {
        return ao ? shEndingsAOOnly[coeff] : shEndingsNormal[coeff];
    }
    ...
    const bool ftraceDebugInfo = false;
    ...
    const int DATATYPE_RGBA16ASRGBM = 0;
    const int DATATYPE_RGBA8 = 1;
    const int DATATYPE_RGBA16ASRGBA32 = 2;
    const int DATATYPE_R8 = 3;
    const int DATATYPE_RGBA32 = 4;
    ...
    public static void ValidateFileAttribs(string file)
    {
    ...
    void ChooseFtraceExe()
    {
        if (rtxMode)
        {
            if (gstorage.runsRTX9)
            {
                if (ftBuildGraphics.traceInstances || !gstorage.runsRTX6)
                {
                    // Can run RTX9 and wants instances - only RTX9 supports them for now
                    // ... or can't even run RTX6
                    ftraceExe = ftraceExe9;
                }
                else
                {
                    // If can run both RTX6 and RTX9, prefer RTX6 for now because it's more tested
                    ftraceExe = gstorage.runsRTX6 ? ftraceExe6 : ftraceExe9;
                }
            }
            else
            {
                // Can't run RTX9 - force RTX6 (since RTX mode is requested)
                ftBuildGraphics.traceInstances = false;
                ftraceExe = ftraceExe6;
            }
        }
        else
        {
            // No RTX mode requested
            ftBuildGraphics.traceInstances = false;
    ...
    public bool TestSystemSpecs()
    {
        ...
        */
        ...
        if (pstorage != null && !pstorage.optimizedLODs && ftraceExe == ftraceExe9)
        {
            pstorage.optimizedLODs = true;
            DebugLogWarning("Forcing Optimized LODs system for the OptiX9 version.");
            EditorUtility.SetDirty(pstorage);
        }
        ...
        if (gstorage != null)
        {
            ...
            if (gstorage.gpuName_2 != SystemInfo.graphicsDeviceName)
            {
    ...
    bool TryExes()
    {
        if (lastReturnValue == 1)
        {
            if (ftraceExe == ftraceExe1)
            {
                ftraceExe = ftraceExe6;
                return true;
            }
            else if (ftraceExe == ftraceExe6)
            {
                ftraceExe = ftraceExe9;
                return true;
            }
    ...
    int GenerateVertexBakedMeshes(int LMID, string lmname, int vertexSamplingDensity, bool hasShadowMask, bool hasDir, bool hasSH, bool monoSH)
    {
        ...
        try
        {
            ...
            errCode = halffloat2vbm(scenePath + "\\" + lmname + (hasSH ? "_final_L0" : "_final_HDR") + (compressedOutput ? ".lz4" : ".dds"), pointer, DATATYPE_RGBA16ASRGBM, 1.0f/lightScale);

            if (hasShadowMask)
                errCode2 = halffloat2vb(scenePath + "\\" + lmname + "_Mask" + (compressedOutput ? ".lz4" : ".dds"), pointerMask, DATATYPE_RGBA8);
            if (hasDir)
                errCode3 = halffloat2vb(scenePath + "\\" + lmname + "_final_Dir" + (compressedOutput ? ".lz4" : ".dds"), pointerDir, DATATYPE_RGBA8);

            if (hasSH)
            {
                errCode4 = halffloat2vb(scenePath + "\\" + lmname + "_final_L1x" + (compressedOutput ? ".lz4" : ".dds"), pointerL1x, DATATYPE_RGBA8);
                errCode5 = halffloat2vb(scenePath + "\\" + lmname + "_final_L1y" + (compressedOutput ? ".lz4" : ".dds"), pointerL1y, DATATYPE_RGBA8);
                errCode6 = halffloat2vb(scenePath + "\\" + lmname + "_final_L1z" + (compressedOutput ? ".lz4" : ".dds"), pointerL1z, DATATYPE_RGBA8);
            }
            ...
            {
                Debug.LogError("hf2vb: " + errCode + " " + errCode2 + " " + errCode3 + " " + errCode4 + " " + errCode5 + " " + errCode6+" "+lmname+" "+compressedOutput);
            }
        }
        finally
        {
    ...
    public void RenderLightProbesButton(bool showMsgWindows = true)
    {
        ...
        if (adaptiveLightScale) lightScale = 1.0f;
        GetUnityWindow();
        showProgressBar = true;
    }
    ...
    public bool TestNeedsBenchmark()
    {
        if (!foundCompatibleSetup && verbose)
        {
            ...
            string question = "Would you like to automatically detect optimal settings for your hardware? You can also do it later via Bakery/Utilities/Detect optimal settings";
            if (gstorage.gpuName_2.Length != 0) question = "This project was previously baked with a different GPU.\n\n" + question;
            int answer = EditorUtility.DisplayDialogComplex("Bakery", question, "Yes", "No", "Never");
            if (answer == 0)
            {
    ...
    static public void ComputeLightScale(float maxEmission)
    {
        lightScale = 1.0f;
        float maxValue = maxEmission;

        var All = FindObjectsOfType(typeof(BakeryLightMesh)) as BakeryLightMesh[];
        for(int i=0; i<All.Length; i++)
        {
            if (All[i].enabled && All[i].intensity > maxValue) maxValue = All[i].intensity;
        }

        var AllP = FindObjectsOfType(typeof(BakeryPointLight)) as BakeryPointLight[];
        for(int i=0; i<AllP.Length; i++)
        {
            if (AllP[i].enabled && AllP[i].intensity > maxValue) maxValue = AllP[i].intensity;
        }

        var All2 = FindObjectsOfType(typeof(BakerySkyLight)) as BakerySkyLight[];
        for(int i=0; i<All2.Length; i++)
        {
            if (All2[i].enabled && All2[i].intensity > maxValue) maxValue = All2[i].intensity;
        }

        var All3 = FindObjectsOfType(typeof(BakeryDirectLight)) as BakeryDirectLight[];
        for(int i=0; i<All3.Length; i++)
        {
            if (All3[i].enabled && All3[i].intensity > maxValue) maxValue = All3[i].intensity;
        }

        Debug.Log("Max light intensity / emission: " + maxValue);

        const float criticalThreshold = 65504;
        if (maxValue > criticalThreshold)
        {
            lightScale = criticalThreshold / maxValue;
            ...
            Debug.LogError("Lights/emissions are brighter than "+criticalThreshold+"! Pre-multiplying all lighting by "+lightScale+".");
        }
    }
    ...
    void OnGUI()
    {
        ...
        GUI.Label(aboutRect, new GUIContent("<color=#5073c9ff><b>Bakery - GPU Lightmapper by Mr F</b></color>", "Version 1.985. Click to go to Bakery Wiki"), linkStyle);
        ...
        if (settingsMode >= SettingsMode.Advanced)
        {
            ftBuildGraphics.splitByTag = GUI.Toggle(new Rect(10, ay, 200, 20), ftBuildGraphics.splitByTag, new GUIContent("Split by baked tag", "Respect 'Baked Tag' in Lightmap Parameters assigned to each mesh renderer. Objects with different tags will always use separate atlases. Additionally enables Named Sections."));
            ay += 20;
            y += 20;
        ...
        else if (settingsMode >= SettingsMode.Advanced)
        {
            ...
            //if (showHacks)
            {
                ...
                if (showAOSettings)
                {
                    ...
                    GUI.Label(new Rect(10+xx, 75+yy, 100, 15), new GUIContent("Affect probes:", "Let AO affect light probes and volumes."));
                    hackAOProbes = EditorGUI.Toggle(new Rect(95+xx, 75+yy, ww, 15), hackAOProbes);
            ...
            adaptiveLightScale =
                GUI.Toggle(new Rect(11, y, 200, 20), adaptiveLightScale,
                    new GUIContent("Adaptive light scale", "Internally scales all light and emissive intensities to fit the highest value in the scene to a reasonable range. Mostly useful for wildly-configured HDRP scenes."));
            y += 20;
            ...
            if (showPerf)
            {
                ...
                prev = ftBuildGraphics.traceInstances;
                if (!gstorage.runsRTX9 || !rtxMode) GUI.enabled = false;
                ftBuildGraphics.traceInstances =
                    GUI.Toggle(new Rect(xx, y, 200, 20), ftBuildGraphics.traceInstances,
                        new GUIContent(" Trace instances", "Optimizes how identical object instances are stored and traced. Currently only works if your GPU supports RTX via OptiX 9."));
                y += 20;
                if (!gstorage.runsRTX9 || !rtxMode) GUI.enabled = prevGUIEnabled;
                if (prev != ftBuildGraphics.traceInstances)
                {
                    ChooseFtraceExe();
                }
                ...
                if (!gstorage.runsNonRTX) GUI.enabled = false;
                ...
                y += 20;
                if (!gstorage.runsNonRTX) GUI.enabled = prevGUIEnabled;

                if (renderDirMode == RenderDirMode.SH)
                {
                    tripleSH = GUI.Toggle(new Rect(xx, y, 200, 20), tripleSH, new GUIContent("Optimize SH maps for batching", "Aka Triple SH.\n\nCreates a long texture atlas with all SH L1 textures and assigns it via the Unity directional map slot, keeping batching. Without this option, SH bakes produce 4 individual maps, 3 of which are assigned via a MaterialPropertyBlock.\n\nShaders need to be aware of the optimization."));
                    y += 20;
                }
                ...
                if (clientMode)
                {
                    ...
                    else
                    {
                        ...
                        if (!ftClient.serverGetDataMode)
                        {
                            ...
                            if (GUI.Button(new Rect(230/2+10, y, 230/2, 30), "Get data") || serverBakeProgress == 2)
                            {
                                serverBakeProgress = 0;
    ...
    public bool SaveRenderSettings()
    {
        ...
        if (
            ...
            renderSettingsStorage.renderSettingsHackAOProbes != hackAOProbes ||
            ...
            renderSettingsStorage.renderSettingsTraceInstances != ftBuildGraphics.traceInstances ||
            renderSettingsStorage.renderSettingsExportTerrainTrees != ftBuildGraphics.exportTerrainTrees ||
            renderSettingsStorage.renderSettingsTripleSH != tripleSH ||
            ...
            renderSettingsStorage.renderSettingsAdaptiveLightScale != adaptiveLightScale ||
    ...
    void LoadVolumes()
    {
        ...
        try
        {
            ...
            bool isAO = EndingAOOnly();
            errCodes[0] = halffloat2vbm(scenePath + "\\volumes_final_" + EndingSH(isAO, 0) + (compressedOutput ? ".lz4" : ".dds"), pointer, DATATYPE_RGBA16ASRGBA32, invMul);
            errCodes[1] = halffloat2vb(scenePath +  "\\volumes_final_" + EndingSH(isAO, 1) + (compressedOutput ? ".lz4" : ".dds"), pointerL1x, DATATYPE_RGBA16ASRGBA32);
            errCodes[2] = halffloat2vb(scenePath +  "\\volumes_final_" + EndingSH(isAO, 2) + (compressedOutput ? ".lz4" : ".dds"), pointerL1y, DATATYPE_RGBA16ASRGBA32);
            errCodes[3] = halffloat2vb(scenePath +  "\\volumes_final_" + EndingSH(isAO, 3) + (compressedOutput ? ".lz4" : ".dds"), pointerL1z, DATATYPE_RGBA16ASRGBA32);
            if (shadowmask)
            {
                errCodes[4] = halffloat2vb(scenePath + "\\volumes_mask" + (compressedOutput ? ".lz4" : ".dds"), pointerShadows, DATATYPE_RGBA8);
            }
            bool ok = true;
            for(int i=0; i<5; i++)
            {
    ...
    static LightingDataAsset CreateEmptyLightingDataAsset()
    {
        ...
        var assetName = GenerateLightingDataAssetName();
        var assetNameFull = assetName + "_probes";
    ...
    IEnumerator RenderReflProbesFunc()
    {
        ...
        for(int s=0; s<sceneCount; s++)
        {
            ...
            {
                var copiedAsset = CreateEmptyLightingDataAsset();
    ...
    public static void WriteSettings(bool _compressedGBuffer, bool _compressedOutput)
    {
        ...
        fsettings.Write(ftraceDebugInfo);
        fsettings.Write(pstorage.multiGPULaunch);
        fsettings.Close();
    }
    ...
    static float GetLightMeshDensity(BakeryLightMesh l)
    {
        var ulht = l.GetComponent<Light>();
        if (ulht != null)
        {
            var size = ftLightMeshInspector.GetAreaLightSize(ulht);
            return (size.x * size.y) / l.samples;
        }

        var mesh = ftBuildGraphics.GetSharedMeshBaked(l.gameObject);
        var verts = mesh.vertices;
        var indices = mesh.triangles;
        var tform = l.transform;
        ...
        for(int j=0; j<tris; j++)
        {
            ...
            totalWorldArea += Vector3.Cross(v2 - v1, v3 - v1).magnitude;
        }

        return (totalWorldArea*0.5f) / l.samples;
    }
    ...
    static int CompareAreaLights(BakeryLightMesh a, BakeryLightMesh b)
    {
        ...
        int compareResult = compB.CompareTo(compA);

        // Due to the same divisor being used for all batched lights, their density must match, so sort them by density 
        if (compareResult == 0)
        {
            if (a.lastDensity == 0) a.lastDensity = GetLightMeshDensity(a);
            if (b.lastDensity == 0) b.lastDensity = GetLightMeshDensity(b);
            compareResult = b.lastDensity.CompareTo(a.lastDensity);
        }
    ...
    public IEnumerator InitializeLightProbes(bool optional)
    {
        ...
        if (useUnityForOcclsusionProbes)
        {
            ...
            UnpatchLightingDataIfRequested();
    ...
    IEnumerator RenderLightProbesFunc()
    {
        ...
        if (anyDirectLightToBake)
        {
...
#endif
                    if (lastReturnValue == 0 || !TryExes()) break;
                }
            ...
            for (int i = 0; i < pointLights.Length; i++)
            {
                ...
                var lights = new BakeryPointLight[1];
                lights[0] = light;
                bool isError = ftBuildLights.BuildLights(lights, 0, 0, null, SampleCount(light.samples), true);
...
#endif
                {
                    ...
                    startInfo.Arguments = "batchpointlight " + scenePathQuoted + " probes.dds " + PASS_HALF + " " + 0 + " " + LMID;
                    DebugLogInfo("Running ftrace " + startInfo.Arguments);
#if LAUNCH_VIA_DLL
                    var crt = ProcessCoroutine(ftraceExe, startInfo.Arguments);
                    while (crt.MoveNext())
    ...
    int SetupLightShadowmaskUsingBitmask(Light ulht, int bitmask, int shadowmaskGroupID, int[] channelBitsPerLayer)
    {
        ...
        {
            if (!shadowmaskGroupIDToChannel.TryGetValue(shadowmaskGroupID, out foundChannel)) foundChannel = -1;
        }
    ...
    bool CollectGroups(List<BakeryLightmapGroup> groupList, List<BakeryLightmapGroup> groupListGIContributing, bool selected, bool probes=false)
    {
        ...
        if (hackAOProbes && hackAOIntensity > 0 && hackAORadius > 0)// ftAdditionalConfig.bakeAOToProbes2)
        {
            // Sort because we must finish all lightmap GI passes before the probe GI pass
            groups.Sort(delegate(BakeryLightmapGroup a, BakeryLightmapGroup b)
            {
                return a.probes.CompareTo(b.probes);
            });
        }
        ...
        else if (probes)
        {
            ...
            {
#if SUPPORTS_APV
                if (!forceRebuildGeometry && apv)
                {
                    DebugLogError("'Export geometry and maps' must be enabled when rendering APVs.");
                    ProgressBarEnd();
                    return false;
                }
                else
    ...
    bool ValidatePrefabs(bool prefabWarning)
    {
        var lmprefabs2 = FindObjectsOfType(typeof(BakeryLightmappedPrefab)) as BakeryLightmappedPrefab[];

        // Remember any prefabs that should be skipped
        ftBuildGraphics.forceScaleInLightmap0 = new HashSet<Renderer>();
        for(int i=0; i<lmprefabs2.Length; i++)
        {
            var p = lmprefabs2[i];
            if (!p.gameObject.activeInHierarchy) continue;
            if (!p.receiveLightmaps)
            {
                var mrs = p.gameObject.GetComponentsInChildren<Renderer>();
                for(int j=0; j<mrs.Length; j++)
                {
                    ftBuildGraphics.forceScaleInLightmap0.Add(mrs[j]);
                }
            }
        }

        if (!prefabWarning) return true;
	...
	public IEnumerator RenderLightmapFunc()
	{
        ...
        if (!ValidatePrefabs(prefabWarning)) yield break;
        ...
        for(int i=0; i<sceneCount; i++)
        {
            ...
            storage.applyViaLightingDataAsset = false;
        ...
        if (forceRebuildGeometry)
        {
            ...
            foreach(var lmgroup in groupList)
            {
                ...
                if (lmgroup.mode != BakeryLightmapGroup.ftLMGroupMode.Vertex || lmgroup.fixPos3D) // skip vertex colored
                {
                    ...
                    if (lmgroup.disableFixPos) continue;
        ...
        if (hackAOIntensity > 0 && hackAOSamples > 0)
        {
            foreach(var lmgroup in groupList)
            {
                ...
                if (doRender) {
                    ...
                    //if (lmgroup.probes && !ftAdditionalConfig.bakeAOToProbes) continue;
                    var rmode = lmgroup.renderMode == BakeryLightmapGroup.RenderMode.Auto ? (int)userRenderMode : (int)lmgroup.renderMode;
                    if (lmgroup.probes && !(hackAOProbes && rmode == (int)RenderMode.AmbientOcclusionOnly)) continue;
        ...
        {
            for(int i=0; i<All.Length; i++) All[i].lastDensity = 0;
        ...
        for(int i=0; i<bounces; i++)
        {
            foreach(var lmgroup in groupList)
            {
                ...
                if (doRender) {
                    ...
                    if (performRendering)
                    {
                        if (lmgroup.probes && !appliedAO && hackAOProbes && hackAOIntensity > 0 && hackAORadius > 0)//ftAdditionalConfig.bakeAOToProbes2)
                        {
                            // Apply AO to last GI outputs of all groups before rendering probe GI, so it reflects the AO
                            foreach(var lmgroup2 in groupList)
                            {
                                if (!RenderLMAOForProbes(lmgroup2))
                                {
                                    ProgressBarEnd();
                                    yield break;
                                }
        ...
        foreach(var lmgroup in groupListPlain)
        {
            ...
            if (doRender) {
                ...
                if (lmgroup.probes && lmgroup.name == "volumes" && lastFoundBakeableVolumes != null && lastFoundBakeableVolumes.Length > 0)
                {
                    ...
                    bool isAO = EndingAOOnly();
                    string l0name =  "volumes_final_" + EndingSH(isAO, 0) + ext;
                    string l1xname = "volumes_final_" + EndingSH(isAO, 1) + ext;
                    string l1yname = "volumes_final_" + EndingSH(isAO, 2) + ext;
                    string l1zname = "volumes_final_" + EndingSH(isAO, 3) + ext;

                    bool pad = denoiseMod == "OIDN2";// && SystemInfo.graphicsDeviceName.Contains("RTX 20"); // lol?
                    ...
                    for(int v=0; v<vols.Length; v++)
                    {
                        ...
                        if (vol.denoise && !EndingAOOnly())
                        {
                            ...
                            startInfo.Arguments      += "\"" + scenePath + "/" + l0name +
                                                     "\" \"" + scenePath + "/" + l1xname +
                                                     "\" \"" + scenePath + "/" + l1yname +
                                                     "\" \"" + scenePath + "/" + l1zname +
                                                     "\" " +
                                                     voffset + " " + rx + " " + ry + " " + (pad ? -rz : rz) + " 32 0";
                            deferredFileSrc.Add("");
                            deferredFileDest.Add("");
                            deferredCommands.Add(startInfo);
                    ...
                    if (clientMode)
                    {
                        ftClient.serverGetFileList.Add(l0name);
                        ftClient.serverGetFileList.Add(l1xname);
                        ftClient.serverGetFileList.Add(l1yname);
                        ftClient.serverGetFileList.Add(l1zname);
                    }
                }
            }
        }
        ...
        if (clientMode)
        {
            ...
            {
                bool isAO = EndingAOOnly();
                ftClient.serverGetFileList.Add("probes_final_" + EndingSH(isAO, 0) + ext);
                ftClient.serverGetFileList.Add("probes_final_" + EndingSH(isAO, 1) + ext);
                ftClient.serverGetFileList.Add("probes_final_" + EndingSH(isAO, 2) + ext);
                ftClient.serverGetFileList.Add("probes_final_" + EndingSH(isAO, 3) + ext);
        ...
        else if (deferredMode)
        {
            ...
            int ftraceFallback = 0;
            bool ftraceFallbackUsed = false;
            ...
            for(int i=0; i<deferredCommands.Count; i++)
            {
                ...
                if (startInfo != null)
                {
                    if (ftraceFallback == 1)
                    {
                        startInfo.FileName = deferredCommands[i].FileName.Replace("ftrace.exe", "ftraceRTX.exe");
                        deferredCommands[i] = startInfo;
                    }
                    else if (ftraceFallback == 2)
                    {
                        startInfo.FileName = deferredCommands[i].FileName.Replace("ftraceRTX.exe", "ftraceRTX9.exe");
                        startInfo.FileName = deferredCommands[i].FileName.Replace("ftrace.exe", "ftraceRTX9.exe");
                        deferredCommands[i] = startInfo;
                    }
                    ...
                    while(errCode2 != 0)
                    {
                        ...
                        if (errCode2 != 0)
                        {
                            ...
                            else if (!ftraceFallbackUsed)
                            {
                                if (errCode2 == 1)
                                {
                                    if (app == "ftrace")
                                    {
                                        DebugLogWarning("Non-RTX lightmapper failed - trying RTX OptiX6 version: " + ftErrorCodes.Translate(app, errCode2));
                                        startInfo.FileName = startInfo.FileName.Replace(ftraceExe1, ftraceExe6);
                                        app = "ftraceRTX";
                                        ftraceFallback = 1;
                                    }
                                    else if (app == "ftraceRTX")
                                    {
                                        DebugLogWarning("RTX OptiX6 lightmapper failed - trying RTX OptiX9 version: " + ftErrorCodes.Translate(app, errCode2));
                                        startInfo.FileName = startInfo.FileName.Replace(ftraceExe6, ftraceExe9);
                                        app = "ftraceRTX9";
                                        ftraceFallback = 2;
                                    }
                                }
                    ...
                    if (ftraceFallback != 0 && !ftraceFallbackUsed)
                    {
                        if (verbose)
                        {
                            if (origApp == "ftrace")
                            {
                                gstorage.runsNonRTX = false;
                                gstorage.runsRTX6 = true;
                                rtxMode = true;
                                EditorUtility.SetDirty(gstorage);
                                ProgressBarEnd();
                                EditorUtility.DisplayDialog("Bakery", "Bakery couldn't run the non-RTX lightmapper and switched to RTX.\nPossibly re-run Bakery -> Utilities -> Detect optimal settings before the next bake.\n\nClick OK to conitnue baking.", "OK");
                            }
                            else if (origApp == "ftraceRTX")
                            {
                                gstorage.runsRTX6 = false;
                                gstorage.runsRTX9 = true;
                                EditorUtility.SetDirty(gstorage);
                                ProgressBarEnd();
                                EditorUtility.DisplayDialog("Bakery", "Bakery couldn't run the RTX (OptiX 6.0) lightmapper because of the Nvidia driver update (>= 590) and switched to RTX (OptiX 9.0).\nPossibly re-run Bakery -> Utilities -> Detect optimal settings before the next bake.\n\nClick OK to conitnue baking.", "OK");
                            }
                        }

                        ftraceFallbackUsed = true;
                    }
                }
        ...
        if (clientMode)
        {
            ...
            if (pstorage == null) pstorage = ftLightmaps.GetProjectSettings();
            if (pstorage.autoServerGetData)
            {
                while(true)
                {
                    if (ftClient.lastServerErrorCode == ftClient.SERVERERROR_BUSY)
                    {
                        serverBakeProgress = 1;
                    }
                    else if (serverBakeProgress == 1 && ftClient.lastServerErrorCode == ftClient.SERVERERROR_IDLE)
                    {
                        serverBakeProgress = 2;
                        break;
                    }
                    yield return null;
                }
    ...
    float FindWindowingLambdaL2(ref SphericalHarmonicsL2 sh, float maxLaplacian)
    {
        ...
        const int iterationLimit = 50000;
    ...
    IEnumerator ApplyBakedData()
    {
        ...
        if (renderProbesNow && hasAnyProbes && !apv)
        {
            var probes = LightmapSettings.lightProbes;
            if (probes == null)
            {
            ...
            try
            {
                ...
                var pointerL1y = handleL1y.AddrOfPinnedObject();
                var pointerL1z = handleL1z.AddrOfPinnedObject();
                errCodes[0] = halffloat2vbm(scenePath + "\\probes_final_" + EndingSH(isAO, 0) + (compressedOutput ? ".lz4" : ".dds"), pointer, DATATYPE_RGBA16ASRGBA32, invMul);
                if (isL2)
                {
                    errCodes[1] = halffloat2vbm(scenePath + "\\probes_final_L1x" + (compressedOutput ? ".lz4" : ".dds"), pointerL1x, DATATYPE_RGBA16ASRGBA32, invMul);
                    errCodes[2] = halffloat2vbm(scenePath + "\\probes_final_L1y" + (compressedOutput ? ".lz4" : ".dds"), pointerL1y, DATATYPE_RGBA16ASRGBA32, invMul);
                    errCodes[3] = halffloat2vbm(scenePath + "\\probes_final_L1z" + (compressedOutput ? ".lz4" : ".dds"), pointerL1z, DATATYPE_RGBA16ASRGBA32, invMul);
                }
                else
                {
                    errCodes[1] = halffloat2vb(scenePath + "\\probes_final_" + EndingSH(isAO, 1) + (compressedOutput ? ".lz4" : ".dds"), pointerL1x, DATATYPE_RGBA16ASRGBA32);
                    errCodes[2] = halffloat2vb(scenePath + "\\probes_final_" + EndingSH(isAO, 2) + (compressedOutput ? ".lz4" : ".dds"), pointerL1y, DATATYPE_RGBA16ASRGBA32);
                    errCodes[3] = halffloat2vb(scenePath + "\\probes_final_" + EndingSH(isAO, 3) + (compressedOutput ? ".lz4" : ".dds"), pointerL1z, DATATYPE_RGBA16ASRGBA32);
                }
                if (isL2)
                {
                    InitL2Postfixes();
                    for(int l=0; l<5; l++)
                    {
                        ...
                        errCodes[4+l] = halffloat2vbm(scenePath + "\\probes_final_" + L2postfix[l] + (compressedOutput ? ".lz4" : ".dds"), pointerL2[l], DATATYPE_RGBA16ASRGBA32, invMul);
                    }
                }
                bool ok = true;
        ...
        if (apv && hasAnyProbes)
        {
            ...
            try
            {
                ...
                int L = -1;
                errCodes[++L] = halffloat2vbm(scenePath + "\\probes_final_L0" + (compressedOutput ? ".lz4" : ".dds"), pointer, DATATYPE_RGBA16ASRGBA32, invMul);
                if (isL2)
                {
                    errCodes[++L] = halffloat2vbm(scenePath + "\\probes_final_L1x" + (compressedOutput ? ".lz4" : ".dds"), pointerL1x, DATATYPE_RGBA16ASRGBA32, invMul);
                    errCodes[++L] = halffloat2vbm(scenePath + "\\probes_final_L1y" + (compressedOutput ? ".lz4" : ".dds"), pointerL1y, DATATYPE_RGBA16ASRGBA32, invMul);
                    errCodes[++L] = halffloat2vbm(scenePath + "\\probes_final_L1z" + (compressedOutput ? ".lz4" : ".dds"), pointerL1z, DATATYPE_RGBA16ASRGBA32, invMul);
                }
                else
                {
                    errCodes[++L] = halffloat2vb(scenePath + "\\probes_final_L1x" + (compressedOutput ? ".lz4" : ".dds"), pointerL1x, DATATYPE_RGBA16ASRGBA32);
                    errCodes[++L] = halffloat2vb(scenePath + "\\probes_final_L1y" + (compressedOutput ? ".lz4" : ".dds"), pointerL1y, DATATYPE_RGBA16ASRGBA32);
                    errCodes[++L] = halffloat2vb(scenePath + "\\probes_final_L1z" + (compressedOutput ? ".lz4" : ".dds"), pointerL1z, DATATYPE_RGBA16ASRGBA32);
                }
                if (isL2)
                {
                    InitL2Postfixes();
                    for(int l=0; l<5; l++) errCodes[++L] = halffloat2vbm(scenePath + "\\probes_final_" + L2postfix[l] + (compressedOutput ? ".lz4" : ".dds"), pointerL2[l], DATATYPE_RGBA16ASRGBA32, invMul);
                }
                if (shadowmask)
                {
                    errCodes[++L] = halffloat2vb(scenePath + "\\probes_mask" + (compressedOutput ? ".lz4" : ".dds"), pointerShadows, DATATYPE_RGBA8);
                }
                if (skyOcclusion)
                {
                    int apvLMID = -1;
                    ...
                    errCodes[++L] = halffloat2vb(scenePath + "\\" + apvSkylightName[apvLMID] + "_RNM0" + (compressedOutput ? ".lz4" : ".dds"), skyPointer, DATATYPE_RGBA16ASRGBA32);
                    errCodes[++L] = halffloat2vb(scenePath + "\\" + apvSkylightName[apvLMID] + "_RNM1" + (compressedOutput ? ".lz4" : ".dds"), skyPointerL1x, DATATYPE_RGBA16ASRGBA32);
                    errCodes[++L] = halffloat2vb(scenePath + "\\" + apvSkylightName[apvLMID] + "_RNM2" + (compressedOutput ? ".lz4" : ".dds"), skyPointerL1y, DATATYPE_RGBA16ASRGBA32);
                    errCodes[++L] = halffloat2vb(scenePath + "\\" + apvSkylightName[apvLMID] + "_RNM3" + (compressedOutput ? ".lz4" : ".dds"), skyPointerL1z, DATATYPE_RGBA16ASRGBA32);
                }
                bool ok = true;
        ...
        foreach(var lmgroup in groupListGIContributingPlain)
        {
            ...
            var shMode3 = shMode && !monoSH && tripleSH;
            ...
            for(int s=0; s<sceneCount; s++)
            {
                ...
                if (dominantDirMode || monoSH || shMode3)
                {
                    ...
                    storage.tripleSH = shMode3;
                }

                if (rnmMode || (shMode && !monoSH && !shMode3))
                {
    ...
    static void PatchLightingDataIfRequested()
    {
#if UNITY_2018_3_OR_NEWER
        if (pstorage.newLightingDataPatch)
        {
            var act = SceneManager.GetActiveScene();
            int sceneCount = EditorSceneManager.sceneCount;
            if (storages == null) CollectStorages();

            for(int s=0; s<sceneCount; s++)
            {
                var scene = EditorSceneManager.GetSceneAt(s);
                if (!scene.isLoaded) continue;
                if (!storages.ContainsKey(scene))
                {
                    CollectStorages();
                    break;
                }
            ...
            for(int s=0; s<sceneCount; s++)
            {
                ...
                SceneManager.SetActiveScene(scene);
                PatchLightingData(scene, storage);

                EditorUtility.SetDirty(storage);

                ftLightmaps.RefreshFull();
            }
            SceneManager.SetActiveScene(act);
        }
#endif
    }

    public static void UnpatchLightingDataIfRequested()
    {
#if UNITY_2018_3_OR_NEWER
        if (pstorage == null) pstorage = ftLightmaps.GetProjectSettings();
        if (pstorage.newLightingDataPatch)
        {
            // Unity deletes lightmap files if its own bake is called, so remove it from the asset
            var act = SceneManager.GetActiveScene();
            int sceneCount = EditorSceneManager.sceneCount;
            if (storages == null) CollectStorages();
            for(int s=0; s<sceneCount; s++)
...
#if UNITY_2018_3_OR_NEWER
    static PropertyInfo inspectorModeProp;

    static void EnableInternalEditForSO(SerializedObject so)
    {
        if (inspectorModeProp == null)
        {
            inspectorModeProp = typeof(SerializedObject).GetProperty("inspectorMode", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            if (inspectorModeProp == null) Debug.LogError("no inspectorMode");
        }
        inspectorModeProp.SetValue(so, InspectorMode.DebugInternal, null);
    }

    static void UnpatchLightingData(Scene scene)
    {
        var ldata = Lightmapping.lightingDataAsset;
        if (ldata == null) return;
        var ldataSO = new SerializedObject(ldata);
        EnableInternalEditForSO(ldataSO);

        var lightmapsProp = ldataSO.FindProperty("m_Lightmaps");
        if (lightmapsProp == null) Debug.LogError("no m_Lightmaps");

        lightmapsProp.arraySize = 0;

        ldataSO.ApplyModifiedPropertiesWithoutUndo();
        ldataSO.Dispose();

        Lightmapping.lightingDataAsset = null;
        Lightmapping.lightingDataAsset = ldata;
    }

    static bool PatchLightingData(Scene scene, ftLightmapsStorage storage)
    {
        var ldata = Lightmapping.lightingDataAsset;
        if (ldata == null)
        {
            ldata = CreateEmptyLightingDataAsset();
        }

        if (ldata == null)
        {
            Debug.LogError("Scene "+scene.name+" has no LightingDataAsset and it cannot be created");
            return false;
        }
        var ldataSO = new SerializedObject(ldata);
        EnableInternalEditForSO(ldataSO);

        var sceneProp = ldataSO.FindProperty("m_Scene");
        if (sceneProp == null)
        {
            Debug.LogError("m_Scene is not present in the LightingDataAsset on this Unity version");
            return false;
        }
        else
        {
        ...
        var rendererDataIDProp = ldataSO.FindProperty("m_LightmappedRendererDataIDs");
        if (rendererDataIDProp == null)
        {
            Debug.LogError("no m_LightmappedRendererDataIDs");
            return false;
        }

        var rendererDataProp = ldataSO.FindProperty("m_LightmappedRendererData");
        if (rendererDataProp == null)
        {
            Debug.LogError("no m_LightmappedRendererData");
            return false;
        }
        ...
        var lightsProp = ldataSO.FindProperty("m_Lights");
        if (lightsProp == null)
        {
            Debug.LogError("no m_Lights");
            return false;
        }

        var bakingOutputsProp = ldataSO.FindProperty("m_LightBakingOutputs");
        if (bakingOutputsProp == null)
        {
            Debug.LogError("no m_LightBakingOutputs");
            return false;
        }

        int numMaps = storage.maps.Count;
        int numDirs = storage.dirMaps != null ? storage.dirMaps.Count : 0;
        int numMasks = storage.masks != null ?  storage.masks.Count : 0;

        lightmapsProp.arraySize = numMaps;
        for(int i=0; i<numMaps; i++)
        {
            var element = lightmapsProp.GetArrayElementAtIndex(i);

            element.FindPropertyRelative("m_Lightmap").objectReferenceValue = storage.maps[i];
            if (i < numDirs) element.FindPropertyRelative("m_DirLightmap").objectReferenceValue = storage.dirMaps[i];
            if (i < numMasks) element.FindPropertyRelative("m_ShadowMask").objectReferenceValue = storage.masks[i];
        }
        ldataSO.FindProperty("m_LightmapsMode").intValue = numDirs > 0 ? (int)LightmapsMode.CombinedDirectional : (int)LightmapsMode.NonDirectional;

        var mrs = storage.bakedRenderers;
        var ids = storage.bakedIDs;
        var scaleOffsets = storage.bakedScaleOffset;
        int numRenderers = mrs.Count;

        var terrains = storage.bakedRenderersTerrain;
        var terrainIDs = storage.bakedIDsTerrain;
        var terrainScaleOffsets = storage.bakedScaleOffsetTerrain;

        rendererDataIDProp.arraySize = rendererDataProp.arraySize = numRenderers + terrains.Count;

        for(int i=0; i<numRenderers; i++)
        {
            if (mrs[i] == null) continue;

            var assetMRIDs = rendererDataIDProp.GetArrayElementAtIndex(i);
            var assetMRData = rendererDataProp.GetArrayElementAtIndex(i);

            var soi = ObjectToSOI(mrs[i]);

            assetMRIDs.Next(true);
            assetMRIDs.longValue = soi.MainLFID;
            assetMRIDs.Next(false);
            assetMRIDs.longValue = soi.PrefabLFID;

            assetMRData.FindPropertyRelative("lightmapIndex").intValue = ids[i];
            assetMRData.FindPropertyRelative("lightmapST").vector4Value = scaleOffsets[i];
            assetMRData.FindPropertyRelative("lightmapIndexDynamic").intValue = usesRealtimeGI ? mrs[i].realtimeLightmapIndex : 65535;
        }
        ...
        for(int i=0; i<terrains.Count; i++)
        {
            ...
            var assetMRIDs = rendererDataIDProp.GetArrayElementAtIndex(numRenderers + i);
            var assetMRData = rendererDataProp.GetArrayElementAtIndex(numRenderers + i);

            var soi = ObjectToSOI(terrains[i]);
            ...
            assetMRData.FindPropertyRelative("lightmapIndex").intValue = terrainIDs[i];
            assetMRData.FindPropertyRelative("lightmapST").vector4Value = terrainScaleOffsets[i];
            assetMRData.FindPropertyRelative("lightmapIndexDynamic").intValue = usesRealtimeGI ? mrs[i].realtimeLightmapIndex : 65535;
        }

#if UNITY_2017_3_OR_NEWER
        int numLights = storage.bakedLights.Count;
        lightsProp.arraySize = bakingOutputsProp.arraySize = numLights;
        for(int i=0; i<numLights; i++)
        {
            ...
            var bakingOutputProp = bakingOutputsProp.GetArrayElementAtIndex(i);
            var lightID = lightsProp.GetArrayElementAtIndex(i);

            bakingOutputProp.FindPropertyRelative("isBaked").boolValue = true;

            var bakeModeProp = bakingOutputProp.FindPropertyRelative("lightmapBakeMode");
            if (channel < 0)
            {
                bakeModeProp.FindPropertyRelative("lightmapBakeType").intValue = (int)LightmapBakeType.Baked;
                bakeModeProp.FindPropertyRelative("mixedLightingMode").intValue = 0;
                bakingOutputProp.FindPropertyRelative("occlusionMaskChannel").intValue = -1;
                bakingOutputProp.FindPropertyRelative("probeOcclusionLightIndex").intValue = 0;
            }
            else
            {
                bakeModeProp.FindPropertyRelative("lightmapBakeType").intValue = (int)LightmapBakeType.Mixed;
                bakeModeProp.FindPropertyRelative("mixedLightingMode").intValue = channel >= 100 ? (int)MixedLightingMode.Subtractive : (int)MixedLightingMode.Shadowmask;
                bakingOutputProp.FindPropertyRelative("occlusionMaskChannel").intValue = channel >= 100 ? (channel-100) : channel;
                bakingOutputProp.FindPropertyRelative("probeOcclusionLightIndex").intValue = storage.bakedLights[i].bakingOutput.probeOcclusionLightIndex;
            }
           

            var soi = ObjectToSOI(storage.bakedLights[i]);

            lightID.Next(true);
            lightID.longValue = soi.MainLFID;
            lightID.Next(false);
        }
#endif
        ...
        storage.applyViaLightingDataAsset = true;

        return true;
    }

    // This function/struct is based on this code by Pema Maling: https://github.com/pema99/GITweaks/blob/master/Editor/GITweaksLightingDataAssetEditor.cs (BSD-3-Clause license)
    ...
    {
        var mainSO = new SerializedObject(obj);
        EnableInternalEditForSO(mainSO);

        long lfid = mainSO.FindProperty("m_LocalIdentfierInFile").longValue;

        var prefabInstance = mainSO.FindProperty("m_PrefabInstance");
        if (prefabInstance != null && prefabInstance.objectReferenceValue != null)
        {
            var prefabInstanceSO = new SerializedObject(prefabInstance.objectReferenceValue);
            EnableInternalEditForSO(prefabInstanceSO);

            var correspondingSO = new SerializedObject(mainSO.FindProperty("m_CorrespondingSourceObject").objectReferenceValue);
            EnableInternalEditForSO(correspondingSO);
    ...
    /*[MenuItem ("Bakery/DEBUG LDATA", false, 0)]
    static void DebugLData()
    {
        PatchLightingData(EditorSceneManager.GetActiveScene(), FindRenderSettingsStorage());
        ...
        for(int lid=0; lid<All.Length; lid++)
        {
            ...
            if (canAppend)
            {
                ...
                for(int i=0; i<savedLights.Count; i++)
                {
                    ...
                    float densityMin = Mathf.Min( obj.lastDensity, savedLights[i].density);
                    float densityMax = Mathf.Max( obj.lastDensity, savedLights[i].density);
                    if (densityMax / densityMin > 1.05f) continue;
    ...
    IEnumerator RenderLMDirect(int LMID, string lmname, int resolution)
    {
        ...
        for(int i=0; i<AllP.Length; i++)
        {
            ...
            {
                Debug.LogError("Non-batched point lights are deprecated.");
    ...
    bool RenderLMAO(int LMID, string lmname)
    {
        ...
        if (hackAOProbes && currentGroup.probes && rmode == (int)RenderMode.AmbientOcclusionOnly) passes |= PASS_RNM0 | PASS_RNM1 | PASS_RNM2 | PASS_RNM3;
        ...
        if (exeMode)
        {
            ...
            else if (hackAOProbes && currentGroup.probes && rmode == (int)RenderMode.AmbientOcclusionOnly)
            {
                renderMode = "aoprobesh";
            }
            else
            {
...
#if UNITY_6000_0_OR_NEWER
        int occlusionMaskChannel = channel > 3 ? -1 : channel; // compute mask channel on our own
    ...
    void PrepareAssetImporting()
    {
        ...
        foreach(var lmgroup in groupListGIContributingPlain)
        {
            ...
            var shMode3 = shMode && !shModeMono && tripleSH;
            if (shModeProbe) shMode = true;
            ...
            //if (!bc6h)
            {
                //if (File.Exists(folder + "../Assets/" + nm + "_final.hdr"))
                {
                    ...
                    if (shMode)
                    {
                        ...
                        desiredTextureType = ftTextureProcessor.TEX_DIR;// TextureImporterType.Default;
                        if (shModeMono || shMode3)
                        {
                            desiredTextureType = shMode3 ? ftTextureProcessor.TEX_DIR_NO_ALPHA_NPOT : ftTextureProcessor.TEX_DIR_NO_ALPHA;
                            outfile = "Assets/" + outputPathCompat + "/"+nm+"_L1" + (pstorage.format8bit == BakeryProjectSettings.FileFormat.PNG ? ".png" : ".tga");
                            ftTextureProcessor.texSettings[outfile] = new Int2(shMode3 ? (dirSize*3+16) : dirSize, desiredTextureType);
                        }
                        else
                        {
                            for(int c=0; c<3; c++)
        ...
        if (UnityEditor.EditorSettings.refreshImportMode == AssetDatabase.RefreshImportMode.OutOfProcessPerQueue)
        {
            ...
            var texS = ftTextureProcessor.texSettings;
    ...
    bool RenderLMAOForProbes(BakeryLightmapGroup lmgroup)
    {
        int LMID = lmgroup.id;
        string lmname = lmgroup.name;
        int lmgroupRenderMode = (int)lmgroup.renderMode;

        System.Diagnostics.ProcessStartInfo startInfo;
        string progressText;

        if (!lightmapHasColor[LMID]) return true;

        var rmode = lmgroupRenderMode == (int)BakeryLightmapGroup.RenderMode.Auto ? (int)userRenderMode : (int)lmgroupRenderMode;

        // Apply AO to the last GI output for probes to reflect
        if (hackAOIntensity > 0 && hackAOSamples > 0 && !lmgroup.probes && rmode != (int)RenderMode.AmbientOcclusionOnly)
        {

            progressText = "Applying AO to " + lmname + " for probes...";
            if (userCanceled) return false;

            var fcomp = new BinaryWriter(File.Open(scenePath + "/addaop_" + LMID + ".bin", FileMode.Create));
            if (clientMode) ftClient.serverFileList.Add("addaop_" + LMID + ".bin");
            fcomp.Write(lmname + "_diffuse_HDR" + (compressedOutput ? ".lz4" : ".dds"));
            ...
            startInfo.FileName        = startInfo.WorkingDirectory + "/" + ftraceExe;
            startInfo.CreateNoWindow = true;
            startInfo.Arguments       =  "addao " + scenePathQuoted + " \"" + lmname + "_diffuse_HDR" + (compressedOutput ? ".lz4" : ".dds")
            + "\"" + " " + PASS_HALF + " " + 0 + " " + LMID + " addaop_" + LMID + ".bin";
    ...
    IEnumerator RenderLMFinalize(int LMID, string lmname, int resolution, bool vertexBake, int lmgroupRenderDirMode, int lmgroupRenderMode, BakeryLightmapGroupPlain lmgroup)
    {
        ...
        if (shMode && lightmapHasRNM[LMID])
        {
            ...
            if (hackAOIntensity > 0 && hackAOSamples > 0 && !lmgroup.probes)//(!lmgroup.probes || ftAdditionalConfig.bakeAOToProbes))
            {
                progressText = "Applying AO to " + lmname + "...";
                var fcomp = new BinaryWriter(File.Open(scenePath + "/addao_" + LMID + ".bin", FileMode.Create));
                if (clientMode) ftClient.serverFileList.Add("addao_" + LMID + ".bin");
                ...
                if (lmgroup.probes)
                {
                    ...
                    fcomp.Write(lmname + "_final_L0" + ext);
                    fcomp.Write(lmname + "_final_L1x" + ext);
                    fcomp.Write(lmname + "_final_L1y" + ext);
                    fcomp.Write(lmname + "_final_L1z" + ext);

                    fcomp.Write(lmname + "_ao_RNM0" + ext);
                    fcomp.Write(lmname + "_ao_RNM1" + ext);
                    fcomp.Write(lmname + "_ao_RNM2" + ext);
                    fcomp.Write(lmname + "_ao_RNM3" + ext);
                }
                else
                {
                    fcomp.Write(lmname + (shMode ? "_final_L0" : "_final_HDR") + (compressedOutput ? ".lz4" : ".dds"));
                ...
                if (lmgroup.probes)
                {
                    startInfo.Arguments       =  "addaosh " + scenePathQuoted + " \"" + lmname + "_final_"
                    + "\"" + " " + PASS_HALF + " " + 0 + " " + LMID + " addao_" + LMID + ".bin";
                }
                else
                {
    ...
    IEnumerator RenderLMEncode(int LMID, string lmname, int resolution, bool vertexBake, int lmgroupRenderDirMode, int lmgroupRenderMode)
    {
        ...
        if (encode)// && !vertexBake)// && File.Exists(scenePath + "/" + lmname + "_final_HDR.dds"))
        {
            ...
            else// if (!bc6h)
            {
                if (!shMode && !rnmMode)
                {
                    ...
                    startInfo.CreateNoWindow = true;
                    startInfo.Arguments       = "\"" + scenePath + "/" + lmname + "_final_HDR" + (compressedOutput ? ".lz4" : ".dds") + "\" \"" + outPath + "\" " + maxValue+" "+(1.0f/ftRenderLightmap.lightScale);
        ...
        if (rnmMode && !vertexBake && lightmapHasRNM[LMID])
        {
            for(int c=0; c<3; c++)
            {
                ...
                startInfo.FileName        = Application.dataPath + "/Editor/x64/Bakery/halffloat2hdr.exe";
                startInfo.CreateNoWindow = true;
                startInfo.Arguments       = "\"" + scenePath + "/" + lmname + "_final_RNM" + c + (compressedOutput ? ".lz4" : ".dds") + "\" \"" + outPath + "\" " + maxValue+" "+(1.0f/ftRenderLightmap.lightScale);
                if (deferredMode)
                {
                    deferredFileSrc.Add("");
                    deferredFileDest.Add("");
        ...
        if (shMode && !vertexBake && lightmapHasRNM[LMID])
        {
            ...
            startInfo.FileName        = Application.dataPath + "/Editor/x64/Bakery/halffloat2hdr.exe";
            startInfo.CreateNoWindow = true;
            startInfo.Arguments       = "\"" + scenePath + "/" + lmname + "_final_L0" + (compressedOutput ? ".lz4" : ".dds") + "\" \"" + outPath + "\" " + maxValue+" "+(1.0f/ftRenderLightmap.lightScale);
            if (deferredMode)
            {
                deferredFileSrc.Add("");
                deferredFileDest.Add("");
            ...
            if (shModeMono || shMode3)
            {
                ...
                if (shMode3) startInfo.Arguments += " t";
    ...
    public void LoadRenderSettings()
    {
        ...
        hackAOProbes = renderSettingsStorage.renderSettingsHackAOProbes;
        ...
        ftBuildGraphics.traceInstances = renderSettingsStorage.renderSettingsTraceInstances;
        ftBuildGraphics.exportTerrainTrees = renderSettingsStorage.renderSettingsExportTerrainTrees;
        tripleSH = renderSettingsStorage.renderSettingsTripleSH;
        ...
        adaptiveLightScale = storage.renderSettingsAdaptiveLightScale;
